...
function initializeCalendar(authToken, userRole) {
    ...
    function fetchFolios(query = '', successCallback, failureCallback) {
        ...
        const url = query ? `https://pasteleria-api-3bno.onrender.com/api/folios?q=${query}` : 'https://pasteleria-api-3bno.onrender.com/api/folios';
    ...
    document.getElementById('dailyFoliosList').addEventListener('change', async (e) => {
        if (e.target.classList.contains('folio-status-check')) {
            ...
            try {
                const response = await fetch(`https://pasteleria-api-3bno.onrender.com/api/folios/${folioId}/status`, {
                    method: 'PATCH',
                    headers: {
    ...
    const handleSearchInput = async (e) => {
        ...
        try {
            const response = await fetch(`https://pasteleria-api-3bno.onrender.com/api/folios?q=${query}`, { headers: { 'Authorization': `Bearer ${authToken}` } });
    ...
    if(searchResultsContainer) {
        searchResultsContainer.addEventListener('click', async (e) => {
            ...
            if (targetItem) {
                ...
                try {
                    const response = await fetch(`https://pasteleria-api-3bno.onrender.com/api/folios/${folioId}`, { headers: { 'Authorization': `Bearer ${authToken}` } });
    ...
    if (printLabelButton) {
        printLabelButton.addEventListener('click', () => {
            ...
            if (currentFolio) {
                ...
                const url = `https://pasteleria-api-3bno.onrender.com/api/folios/${currentFolio.id}/label-pdf?token=${currentToken}`;
    ...
    if(cancelFolioButton){
        cancelFolioButton.addEventListener('click', async () => {
            ...
            if (currentFolio && confirm(`¿Estás seguro de que deseas CANCELAR el folio ${currentFolio.folioNumber}?`)) {
                ...
                try {
                    const response = await fetch(`https://pasteleria-api-3bno.onrender.com/api/folios/${currentFolio.id}/cancel`, {
                        method: 'PATCH',
                        headers: { 'Authorization': `Bearer ${authToken}` }
                    });
        ...
        deleteFolioButton.addEventListener('click', async () => {
            ...
            if (currentFolio) {
                if (confirm(`¿Estás seguro de que deseas eliminar el folio ${currentFolio.folioNumber}? Esta acción no se puede deshacer.`)) {
                    ...
                    try {
                        const response = await fetch(`https://pasteleria-api-3bno.onrender.com/api/folios/${folioId}`, {
                            method: 'DELETE',
                            headers: { 'Authorization': `Bearer ${authToken}` }
                        });
    ...
    if (printLabelsButton) {
        printLabelsButton.addEventListener('click', () => {
            ...
            if (date) {
                const currentToken = localStorage.getItem('authToken');
                const url = `https://pasteleria-api-3bno.onrender.com/api/folios/day-summary-pdf?type=labels&date=${date}&token=${currentToken}`;
                window.open(url, '_blank');
            }
        });
    }
    ...
    if (printOrdersButton) {
        printOrdersButton.addEventListener('click', () => {
            ...
            if (date) {
                const currentToken = localStorage.getItem('authToken');
                const url = `https://pasteleria-api-3bno.onrender.com/api/folios/day-summary-pdf?type=orders&date=${date}&token=${currentToken}`;
                window.open(url, '_blank');
            }
        });
    }
